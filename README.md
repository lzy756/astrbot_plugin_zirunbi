...
为了防止插件更新时造成数据丢失（如用户的持仓、余额、交易记录等），请遵循以下指南：

1.  **数据备份**：
    `zirunbi.db` 文件包含了所有用户数据。在更新插件文件之前，**强烈建议**先将此文件备份到安全位置。

2.  **安全更新**：
    *   下载新版本插件代码。
    *   覆盖安装时，请**保留原有的 `zirunbi.db` 文件**。如果该文件被覆盖或删除，数据将永久丢失。
    *   如果新版本包含数据库结构变更，插件会自动尝试迁移。
    *   也可以手动运行 `python migrate.py` 脚本，它会自动备份并尝试安全迁移数据库。

3.  **手动迁移**：
    如果遇到数据库相关的错误，可以尝试运行目录下的迁移脚本：
    ```bash
    python migrate.py
    ```
